...
IF (ALLOCATED(aBucket%Data)) THEN
  WRITE(UNIT_SCREEN,'(2X,A,1X,I0)',ADVANCE='YES') "Bucket:", iBucket
ELSE
  WRITE(UNIT_SCREEN,'(2X,A,1X,I0,1X,A,1X)',ADVANCE='NO') "Bucket:", iBucket, ":"
END IF

DO WHILE(ASSOCIATED(aBucket))
  IF (ALLOCATED(aBucket%Data)) THEN
    nData = SIZE(aBucket%Data)
    WRITE(FormatString,'(A,I0,A)') '(2X,A,I0,A,', nData, '(2X,I0))'
    WRITE(UNIT_SCREEN,FormatString,ADVANCE='YES') "BucketID: ", aBucket%ID, " - Data: ", aBucket%Data(1:nData)
  ELSE
    nData = 0
    WRITE(FormatString,'(A,I0,A)') '(', nData+1, '(2X,I0))'
