...
IF (.NOT. ALLOCATED(MeshData_MasterSlavesToElements)) THEN
  nQuad4Quad = 0
ELSE 
  nQuad4Quad = SIZE(MeshData_MasterSlavesToElements,2)
END IF
...
IF (Level .GT. 1) THEN
  DO iLevel=Level-1,1,-1
    DO iQuad4Quad=1,nQuad4Quad
      iElem(1:5) = MeshData_MasterSlavesToElements(1:5,iQuad4Quad)
...
IF (.NOT. ALLOCATED(MeshData_MasterSlavesToElements)) THEN
  nQuad2Quad = 0
ELSE 
  nQuad2Quad = SIZE(MeshData_MasterSlavesToElements,2)
END IF
...
IF (Level .GT. 1) THEN
  DO iLevel=Level-1,1,-1
    DO iQuad2Quad=1,nQuad2Quad
      iElem(1:3) = MeshData_MasterSlavesToElements(1:3,iQuad2Quad)
      IF (((MeshData_ElementsToRefineFlag(iElem(2)) .EQV. .TRUE.) .OR. &
