...
!
!======================================================================================================================!
MODULE MOD_NonConformingConnectivity
!----------------------------------------------------------------------------------------------------------------------!
USE MOD_GLOBAL_vars
!----------------------------------------------------------------------------------------------------------------------!
IMPLICIT NONE
...
PRIVATE
!----------------------------------------------------------------------------------------------------------------------!
INTERFACE GetNonConformingSidesArray
  MODULE PROCEDURE GetNonConformingSidesArray
END INTERFACE

INTERFACE GetQuad2QuadArray
  MODULE PROCEDURE GetQuad2QuadArray
END INTERFACE

INTERFACE GetQuad4QuadArray
  MODULE PROCEDURE GetQuad4QuadArray
END INTERFACE
!----------------------------------------------------------------------------------------------------------------------!
PUBLIC :: GetNonConformingSidesArray
PUBLIC :: GetQuad2QuadArray
PUBLIC :: GetQuad4QuadArray
!----------------------------------------------------------------------------------------------------------------------!
!
!
!
...
USE MOD_MeshMain_vars,ONLY: MeshData_MasterSlavesToFaces
...
IF (nQuad2Quad .GT. 0) THEN
  ...
  DO WHILE (iQuad2Quad .LE. nQuad2Quad)
    ...
    iQuad2Quad = iQuad2Quad+1
  END DO
  IF (ALLOCATED(MeshData_MasterSlavesToFaces)) THEN
    DEALLOCATE(MeshData_MasterSlavesToFaces)
  END IF
  ALLOCATE(MeshData_MasterSlavesToFaces(1:2+1,1:nQuad2Quad))

  IF (ALLOCATED(MeshData_MasterSlavesToElements)) THEN
    DEALLOCATE(MeshData_MasterSlavesToElements)
  END IF

  ALLOCATE(MeshData_MasterSlavesToElements(1:2+1,1:nQuad2Quad))
  iQuad2Quad = 1
  DO WHILE (iQuad2Quad .LE. nQuad2Quad)
    Flag = .FALSE.
    IF (ALL(Quad2QuadAUX(iQuad2Quad,1:4) .EQ. MasterSideAUX(iQuad2Quad,1:4))) THEN
    ...
    END IF
    IF (Flag .EQV. .TRUE.) THEN
      MeshData_MasterSlavesToFaces(1,iQuad2Quad) = MasterSidesArray(MasterSideAUX(iQuad2Quad,5),DATA_SideID)
      MeshData_MasterSlavesToFaces(2,iQuad2Quad) = SlaveSidesArray(Quad2QuadArray(Quad2QuadAUX(iQuad2Quad,5),7),DATA_SideID)
      MeshData_MasterSlavesToFaces(3,iQuad2Quad) = SlaveSidesArray(Quad2QuadArray(Quad2QuadAUX(iQuad2Quad,5),8),DATA_SideID)

      MeshData_MasterSlavesToElements(1,iQuad2Quad) = MasterSidesArray(MasterSideAUX(iQuad2Quad,5),DATA_ElemID)
      MeshData_MasterSlavesToElements(2,iQuad2Quad) = SlaveSidesArray(Quad2QuadArray(Quad2QuadAUX(iQuad2Quad,5),7),DATA_ElemID)
      MeshData_MasterSlavesToElements(3,iQuad2Quad) = SlaveSidesArray(Quad2QuadArray(Quad2QuadAUX(iQuad2Quad,5),8),DATA_ElemID)
    END IF
    iQuad2Quad = iQuad2Quad+1
  END DO
  IF (ParametersMeshElementsList%DebugMeshElementsList .EQV. .TRUE.) THEN
    ...
    DO iQuad2Quad=1,nQuad2Quad
      WRITE(UNIT_SCREEN,FormatString) iQuad2Quad, MeshData_MasterSlavesToNodes(1:6,iQuad2Quad)
    END DO
    WRITE(UNIT_SCREEN,*)
    WRITE(UNIT_SCREEN,*)
    ...
    DO iQuad2Quad=1,nQuad2Quad
      WRITE(UNIT_SCREEN,FormatString) iQuad2Quad, MeshData_MasterSlavesToElements(1:3,iQuad2Quad)
    END DO
  END IF
END IF
...
IF (nQuad4Quad .GT. 0) THEN
  ...
  IF (ALLOCATED(MeshData_MasterSlavesToFaces)) THEN
    DEALLOCATE(MeshData_MasterSlavesToFaces)
  END IF
  ALLOCATE(MeshData_MasterSlavesToFaces(1:4+1,1:nQuad4Quad))

  IF (ALLOCATED(MeshData_MasterSlavesToElements)) THEN
    DEALLOCATE(MeshData_MasterSlavesToElements)
  END IF
  ALLOCATE(MeshData_MasterSlavesToElements(1:4+1,1:nQuad4Quad))
  ...
  DO WHILE (iQuad4Quad .LE. nQuad4Quad)
    ...
    END IF
    IF (Flag .EQV. .TRUE.) THEN
      MeshData_MasterSlavesToFaces(1,iQuad4Quad) = MasterSidesArray(MasterSideAUX(iQuad4Quad,5),DATA_SideID)
      MeshData_MasterSlavesToFaces(2,iQuad4Quad) = SlaveSidesArray(Quad4QuadArray(Quad4QuadAUX(iQuad4Quad,5),10),DATA_SideID)
      MeshData_MasterSlavesToFaces(3,iQuad4Quad) = SlaveSidesArray(Quad4QuadArray(Quad4QuadAUX(iQuad4Quad,5),11),DATA_SideID)
      MeshData_MasterSlavesToFaces(4,iQuad4Quad) = SlaveSidesArray(Quad4QuadArray(Quad4QuadAUX(iQuad4Quad,5),12),DATA_SideID)
      MeshData_MasterSlavesToFaces(5,iQuad4Quad) = SlaveSidesArray(Quad4QuadArray(Quad4QuadAUX(iQuad4Quad,5),13),DATA_SideID)

      MeshData_MasterSlavesToElements(1,iQuad4Quad) = MasterSidesArray(MasterSideAUX(iQuad4Quad,5),DATA_ElemID)
      MeshData_MasterSlavesToElements(2,iQuad4Quad) = SlaveSidesArray(Quad4QuadArray(Quad4QuadAUX(iQuad4Quad,5),10),DATA_ElemID)
      MeshData_MasterSlavesToElements(3,iQuad4Quad) = SlaveSidesArray(Quad4QuadArray(Quad4QuadAUX(iQuad4Quad,5),11),DATA_ElemID)
      MeshData_MasterSlavesToElements(4,iQuad4Quad) = SlaveSidesArray(Quad4QuadArray(Quad4QuadAUX(iQuad4Quad,5),12),DATA_ElemID)
      MeshData_MasterSlavesToElements(5,iQuad4Quad) = SlaveSidesArray(Quad4QuadArray(Quad4QuadAUX(iQuad4Quad,5),13),DATA_ElemID)
    END IF
    iQuad4Quad = iQuad4Quad+1
  END DO
  IF (ParametersMeshElementsList%DebugMeshElementsList .EQV. .TRUE.) THEN
    ...
    DO iQuad4Quad=1,nQuad4Quad
      WRITE(UNIT_SCREEN,FormatString) iQuad4Quad, MeshData_MasterSlavesToNodes(1:9,iQuad4Quad)
    END DO
    WRITE(UNIT_SCREEN,*)
    WRITE(UNIT_SCREEN,*)
    ...
    DO iQuad4Quad=1,nQuad4Quad
      WRITE(UNIT_SCREEN,FormatString) iQuad4Quad, MeshData_MasterSlavesToElements(1:5,iQuad4Quad)
    END DO
  END IF
END IF
...
!
!----------------------------------------------------------------------------------------------------------------------!
END MODULE MOD_NonConformingConnectivity
!======================================================================================================================!
