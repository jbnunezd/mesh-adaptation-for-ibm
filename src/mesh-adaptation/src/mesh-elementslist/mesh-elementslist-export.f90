...
DO WHILE (ASSOCIATED(aElem))
  ...
  DO WHILE(ASSOCIATED(aSide))
    IF (ASSOCIATED(aSide%BC) .EQV. .TRUE.) THEN
      ...
      END DO
      MeshData_BCFacesToType(BCFaceID) = aSide%BC%BCMark
    END IF
...
INTEGER :: LocSide
INTEGER :: FaceNodes(1:nFaceNodes)
INTEGER :: aFaceNodes(1:nFaceNodes)
INTEGER :: bFaceNodes(1:nFaceNodes)
!----------------------------------------------------------------------------------------------------------------------!
TYPE(tElem),POINTER :: aElem
TYPE(tSide),POINTER :: aSide
!----------------------------------------------------------------------------------------------------------------------!
...
DO WHILE (ASSOCIATED(aElem))
  ...
  DO WHILE (ASSOCIATED(aSide))
    IF (ASSOCIATED(aSide%BC) .EQV. .FALSE.) THEN
      ...
      LocSide = aSide%LocSide
      DO iNode=1,aSide%nNodes
        aFaceNodes(iNode) = aSide%Nodes(iNode)%Node%NodeID
      END DO
      bFaceNodes(1:nFaceNodes) = MeshData_FacesToNodes(1:nFaceNodes,SideID)
      IF (ALL((aFaceNodes-bFaceNodes) .EQ. 0)) THEN
        MeshData_ElementsToFaces(LocSide,ElemID) = +MeshData_ElementsToFaces(LocSide,ElemID)
      ELSE
        MeshData_ElementsToFaces(LocSide,ElemID) = -MeshData_ElementsToFaces(LocSide,ElemID)
      END IF
    END IF
