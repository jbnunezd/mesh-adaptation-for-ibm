...
CALL BuildHexaMap(PP_NGeo,nElemNodes,HexaMap)
    ...
    DO ii=1,nBoxElems3D(1)
      ...
      DO iVertex=1,nElemNodes
        ...
        IF (LowerCase(ExtrusionDirection) .EQ. 'y') THEN
          i = HexaMap(ElemNodeIDSortingForExtrusion(VertexMap(iVertex)),1)
          j = HexaMap(ElemNodeIDSortingForExtrusion(VertexMap(iVertex)),2)
          k = HexaMap(ElemNodeIDSortingForExtrusion(VertexMap(iVertex)),3)
          Nodes(i,j,k)%Node => MeshNodes(NodeID)%Node
        ELSEIF (LowerCase(ExtrusionDirection) .EQ. 'z') THEN
          i = HexaMap(VertexMap(iVertex),1)
          j = HexaMap(VertexMap(iVertex),2)
          k = HexaMap(VertexMap(iVertex),3)
          Nodes(i,j,k)%Node => MeshNodes(NodeID)%Node
        END IF
      END DO
